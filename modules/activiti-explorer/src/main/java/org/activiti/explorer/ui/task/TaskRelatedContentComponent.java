...
public class TaskRelatedContentComponent extends VerticalLayout implements
	...
	public TaskRelatedContentComponent(Task task,
		...
		this.attachmentRendererManager = ExplorerApp.get()
	...
	public void refreshTaskAttachments() {
		...
		if (task.getProcessInstanceId() != null) {
			attachments = (taskService.getProcessInstanceAttachments(task
					.getProcessInstanceId()));
			ProcessInstance parentProcess = runtimeService
					.createProcessInstanceQuery()
					.subProcessInstanceId(task.getProcessInstanceId())
					.singleResult();
			// List <HistoricProcessInstance> historicProcesses =
			// historyService.createHistoricProcessInstanceQuery().superProcessInstanceId(task.getProcessInstanceId()).list();
			if (parentProcess != null) {
				attachments.addAll(taskService
						.getProcessInstanceAttachments(parentProcess.getId()));
				ProcessInstance parentProcessSecondLevel = runtimeService
						.createProcessInstanceQuery()
						.subProcessInstanceId(
								parentProcess.getProcessInstanceId())
						.singleResult();
				if (parentProcessSecondLevel != null) {
					attachments
							.addAll(taskService
									.getProcessInstanceAttachments(parentProcessSecondLevel
											.getId()));
				}
			}
			/*
			 ...
			 * attachments
			 * .addAll(taskService.getProcessInstanceAttachments(aSubProcess
			 * .getId())); }
			 */
		} else {
